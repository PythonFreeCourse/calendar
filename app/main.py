...
def add_countries_to_db():
    ...
    Information is based on the "countries" list, located in app/database/countries.py.
    Names are described either as "Country Name, City Name" or "Country Name" Name solely.
...
async def time(
    ...
    By Click on country name the page transfers to "/global_time/{country_name}/{chosen_datetime}".
...
@app.get("/global_time/{country_name}/{chosen_datetime}/{ip}")
async def time_conv(
    ip,
    country_name,
    ...
    session=Depends(get_db)):
    """ 
    Displays the chosen country name and the meeting time converted to it's local time.
    If the Users IP is not recognized , displays error message
    *** Temporarily uses a random Israeli ip, while using development environment ***
    """
    error_msg = "Your ip address is not associated with any geographical location. This function cannot operate."
    # random_israeli_ip = '85.250.214.253'
    match = geolite2.lookup(ip)
    if match is not None:
        ip_timezone = match.timezone
        target_timezone = session.query(Country.timezone).filter_by(name=country_name).first()[0]
        meeting_time_with_utc = pytz.timezone(ip_timezone).localize(chosen_datetime)
        target_with_utc = pytz.timezone(target_timezone)
