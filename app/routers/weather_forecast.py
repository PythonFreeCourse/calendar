...
def get_historical_weather(input_date, location):
    ...
    input_query_string["endDateTime"] = (input_date + datetime.timedelta(days=1)).isoformat()
    input_query_string["location"] = location
    api_json, error_text = get_data_from_weather_api(HISTORY_URL, input_query_string)
    location_found = list(api_json.keys())[0]
    if api_json:
        weather_data = {'MinTempCel': api_json[location_found]['values'][0]['mint'],
...
def get_history_relevant_year(day, month):
    """ return the relevant year in order to call the get_historical_weather function with.
    ...
    """
    try:
        relevant_date = datetime.datetime(year=datetime.datetime.now().year, month=month, day=day)
    except ValueError:  # only if the day & month are 29.02 and there is no such date this year
        relevant_date = datetime.datetime(year=datetime.datetime.now().year, month=month, day=day - 1)
...
def get_forecast_by_historical_data(day, month, location):
    ...
    try:
        input_date = datetime.datetime(year=relevant_year, month=month, day=day)
    except ValueError:
        # only if the day & month are 29.02 and there is no such date on the relevant year
        input_date = datetime.datetime(year=relevant_year, month=month, day=day - 1)
...
def get_forecast(requested_date, location):
    ...
    if forecast_type == HISTORY_TYPE:
        weather_json, error_text = get_historical_weather(requested_date, location)
    if forecast_type == FORECAST_TYPE:
        weather_json, error_text = get_forecast_weather(requested_date, location)
...
def get_weather_data(requested_date, location):
    ...
    """
    output = {}
    requested_date = datetime.date(requested_date.year, requested_date.month, requested_date.day)
    ...
    if valid_input:
        ...
        if error_text:
            ...
            output["MinTempFar"] = round((weather_json['MinTempCel'] * 9/5) + 32)
            output["MaxTempFar"] = round((weather_json['MaxTempCel'] * 9/5) + 32)
