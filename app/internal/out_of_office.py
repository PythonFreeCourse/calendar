...
def get_who_is_out_of_office(session: Session, event_start_date: datetime,
    ...
    """
    out_of_office_users = session.query(
        User.username, User.email).join(OutOfOffice).filter(
            User.email.in_(invited_emails)).filter(
...
def insert_new_out_of_office(out_of_office_data, user: User, session: Session):
    out = get_out_of_office_template(
        1,
        user_id=user.id,
        start_date=get_date_formatted(out_of_office_data, START_DATE,
                                      START_TIME, DATETIME_FORMAT),
        end_date=get_date_formatted(out_of_office_data, END_DATE, END_TIME,
                                    DATETIME_FORMAT),
        status='On')
    session.add(out)
...
def get_out_of_office_template(out_of_office_id,
                               ...
                               end_date=None,
                               status='Off'):
    return OutOfOffice(id=out_of_office_id,
                       user_id=user_id,
                       start_date=start_date,
                       end_date=end_date,
                       status=status)
...
def get_date_formatted(out_of_office_data, date, time, date_format):
    return datetime.strptime(
        out_of_office_data[date] + ' ' + out_of_office_data[time], date_format)
...
def update_out_of_office(out_of_office_data_from_req,
    ...
    if out_of_office_data_from_req['outOfOffice'] == activate_out_of_office:
        out_of_office_data_from_db.start_date = get_date_formatted(
            out_of_office_data_from_req, START_DATE, START_TIME,
            DATETIME_FORMAT)
        out_of_office_data_from_db.end_date = get_date_formatted(
            out_of_office_data_from_req, END_DATE, END_TIME, DATETIME_FORMAT)
