...
def get_parashot_from_json() -> List[Dict[str, Optional[str]]]:
    ...
    Each year the Json file will need to be re-updated according to this API in this way:

    def relevent_details(p):
    parashot_dict = {'name': p['title'], 'hebrew': p['hebrew'], 'link': p['link'], 'date': p['date']}
    return parashot_dict


    def get_all_parashot():
    r = requests.get('https://www.hebcal.com/hebcal?v=1&cfg=json&maj=on&min=on&mod=on&nx=on&year=now&month=x&ss=on'
                     '&mf=on&c=on&geo=geoname&geonameid=293397&m=50&s=on')
    items = r.json()['items']
