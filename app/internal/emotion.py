...
from app.config import CONTENT_WEIGHTS, LEVEL_OF_SIGNIFICANCE, TITLE_WEIGHTS
...
Emoticon = NamedTuple(
    "Emoticon",
    [("dominant", str), ("score", float), ("code", str)],
)
...
def get_weight(
    emotion: str,
    title_emotion: Dict[str, float],
    content_emotion: Dict[str, float] = None,
) -> float:
    if not content_emotion:
        return title_emotion[emotion]
    return (
        title_emotion[emotion] * TITLE_WEIGHTS
...
def is_emotion_above_significance(
    dominant_emotion: Emoticon,
    significance: float = LEVEL_OF_SIGNIFICANCE,
) -> bool:
    """
