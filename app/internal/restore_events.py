...
def delete_events_after_optionals_num_days(days, session):
    ...
    user_events_ids_to_be_deleted = (session.query(UserEvent).join(Event).
                                     filter(
        Event.deleted_date < date_to_delete).all())

    for id_to_be_deleted in user_events_ids_to_be_deleted:
        (session.query(UserEvent).filter(UserEvent.id ==
                                         id_to_be_deleted.id).delete())
...
def get_events_ids_to_restored(events_data: List) -> List[str]:
    ...
    for element, element_value in events_data:
        if is_checkbox_element_is_on:
            ...
            is_checkbox_element_is_on = False
        if (element == check_element_name
                and element_value == check_element_on_value):
