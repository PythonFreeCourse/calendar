...
def validate_input(db: Session, userid: int, start: datetime.datetime,
    ...
    if not does_user_exist(session=db, user_id=userid):
        return False, INVALID_USER, start, end
    if start is None:
        start = datetime.datetime(year=datetime.datetime.now().year,
    ...
    if start >= end:
        return False, INVALID_DATE_RANGE, start, end
    return True, "", start, end


def get_events_count_stats(db: Session, userid: int, start: datetime.datetime,
                           end: datetime.datetime)\
    ...
    """
    return {"events_count_stats":
            {"meetings_for_user": db.query(Event.id).
                join(UserEvent, UserEvent.event_id == Event.id).
                filter(UserEvent.user_id == userid).filter
                (or_(and_(Event.start >= start, Event.start <= end),
                     and_(Event.end >= start, Event.end <= end),
                     and_(Event.start <= start, Event.end >= end))).count(),
             "created_by_user": db.query(Event.id).
                filter(Event.owner_id == userid).filter
                (or_(and_(Event.start >= start, Event.start <= end),
                     and_(Event.end >= start, Event.end <= end),
                     and_(Event.start <= start, Event.end >= end))).count()}
            }
...
def get_daily_events_statistics(db: Session, userid: int,
    ...
    """
    events_by_date = db.query(func.date(Event.start),
                              func.count(func.date(Event.start))).\
        join(UserEvent, UserEvent.event_id == Event.id).\
        filter(UserEvent.user_id == userid).\
        filter(or_(and_(Event.start >= start, Event.start <= end),
                   and_(Event.end >= start, Event.end <= end),
                   and_(Event.start <= start, Event.end >= end)))\
        .group_by(func.date(Event.start)).all()
...
def get_events_duration_statistics(db: Session, userid: int,
                                   start: datetime.datetime,
                                   end: datetime.datetime)\
...
def get_participants_statistics(db: Session, userid: int,
                                start: datetime.datetime,
                                end: datetime.datetime)\
        -> Dict[str, Dict[str, str]]:
    ...
    """
    subquery = db.query(Event.id).\
        join(UserEvent, UserEvent.event_id == Event.id).\
        filter(UserEvent.user_id == userid).\
        filter(or_(and_(Event.start >= start, Event.start <= end),
                   and_(Event.end >= start, Event.end <= end),
                   and_(Event.start <= start, Event.end >= end))).subquery()
    event_participants = db.query(UserEvent.user_id,
                                  func.count(UserEvent.user_id)).\
        filter(UserEvent.user_id != userid).\
        filter(UserEvent.event_id.in_(subquery)).group_by(UserEvent.user_id).\
        order_by(func.count(UserEvent.user_id).desc()).first()
    if event_participants:
        return {"participants_statistics": {"max_events":
                event_participants[1], "participant_name":
...
def prepare_display_text(output: Dict[str, Union[datetime.datetime, int, str]],
                         start: datetime.datetime,
                         end: datetime.datetime) -> Dict[str, Dict[str, str]]:
...
def get_statistics(db: Session, userid: int, start: datetime.datetime = None,
    ...
    """
    output = {}
    valid_input, error_text, start, end =\
        validate_input(db, userid, start, end)
    if not valid_input:
        output["start"] = start
        output["end"] = end
