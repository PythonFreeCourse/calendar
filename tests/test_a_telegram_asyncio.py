...
from tests.fixtures.asyncio_fixture import today_date
...
async def test_bot_model():
    ...
    assert (
        bot.webhook_setter_url
        == "https://api.telegram.org/botfake \
...
class TestHandlers:
    ...
    async def test_start_handlers(self):
        ...
        assert (
            await message.process_callback()
            == """Hello, Moshe!
Welcome to PyLendar telegram client!"""
    ...
    async def test_default_handlers(self):
        wrong_start = MessageHandler(
            Chat(gen_message("start")),
            self.TEST_USER,
        )
        wrong_show_events = MessageHandler(
            Chat(gen_message("show_events")),
            self.TEST_USER,
        )
