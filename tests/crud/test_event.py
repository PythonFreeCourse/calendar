...
def test_get_change_owner(
    ...
    original_owner_own_events = user_crud.get_events(
        session_v2,
        original_owner,
    )
    original_owner_events = user_crud.get_owned_events(
        session_v2,
        original_owner,
    )
    ...
    assert (
        len(original_owner_events) == 1 and len(original_owner_own_events) == 1
    )
    ...
    new_owner_own_events = user_crud.get_events(session_v2, user2)
    new_owner_events = user_crud.get_owned_events(session_v2, user2)

    # Verify new owner changed and values are updated to 1.
    assert new_owner == user2
    assert len(new_owner_events) == 1 and len(new_owner_own_events) == 1
    ...
    # Verify original_owner values are now 0.
    assert (
        len(original_owner_own_events) == 0 and len(original_owner_events) == 0
    )
