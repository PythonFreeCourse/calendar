...
def create_test_form(
    form_dict: bool = False, **kwargs: Dict[str, str]
) -> meds.Form:
    form = WEB_FORM.copy()
...
FORM_TRANS = [
    (
        ...
        meds.Form(
            ...
            min=time(4),
            max=time(6),
            start=datetime(2015, 10, 21, 8),
            end=datetime(2015, 10, 22, 22),
            note=QUOTE,
        ),
        {
            "name": NAME,
    ...
    (
        create_test_form(form_dict=True, first="13:30"),
        meds.Form(
            name=NAME,
            first=time(13, 30),
            amount=3,
            ...
            min=time(4),
            max=time(6),
            start=datetime(2015, 10, 21, 13, 30),
            end=datetime(2015, 10, 22, 22),
            note=QUOTE,
        ),
        {
...
CREATE_FIRST = [
    (
        create_test_form(first="10:45"),
        time(15),
        datetime(2015, 10, 21, 10, 45),
    ...
    (
        create_test_form(first="20:30", late="02:00"),
        time(1),
        datetime(2015, 10, 21, 20, 30),
        datetime(2015, 10, 22, 1),
    ),
    (
        create_test_form(first="21:30", late="02:00"),
        time(1),
        datetime(2015, 10, 21, 21, 30),
        datetime(2015, 10, 22, 1, 30),
    ),
    (
        create_test_form(first="16:30", late="02:00"),
        time(10),
        datetime(2015, 10, 21, 16, 30),
        None,
    ),
]
...
FIRST = [
    ...
    (
        ...
        [
            ...
            datetime(2015, 10, 21, 17, 30),
            datetime(2015, 10, 21, 21, 30),
        ],
    ),
    (
        create_test_form(first="17:20"),
        [time(10), time(15), time(20)],
    ...
    (
        ...
        [
            datetime(2015, 10, 21, 16, 43),
            datetime(2015, 10, 21, 20, 43),
...
REMINDERS = [
    (
        [time(10), time(15), time(20)],
        time(8),
        datetime(2015, 10, 21, 8),
        0,
        datetime(2015, 10, 22, 22),
        [
            datetime(2015, 10, 21, 10),
            datetime(2015, 10, 21, 15),
            datetime(2015, 10, 21, 20),
        ],
    ),
    (
        ...
        datetime(2015, 10, 22, 22),
        [
            datetime(2015, 10, 22, 10),
            datetime(2015, 10, 22, 15),
...
def test_get_interval_in_minutes(
    early: time,
    late: time,
    interval: int,
) -> None:
...
def test_calc_reminder_interval_in_seconds(
    form: meds.Form,
    interval: int,
) -> None:
...
def test_validate_first_day_reminder(
    previous: datetime,
    reminder_time: time,
    minimum: time,
    maximum: time,
    ...
    assert (
        meds.validate_first_day_reminder(
            previous,
            reminder_time,
            minimum,
            maximum,
...
def test_create_first_day_reminder(
    form: meds.Form,
    time_obj: time,
    previous: datetime,
    reminder: datetime,
...
def test_get_first_day_reminders(
    form: meds.Form,
    times: List[time],
...
def test_get_reminder_datetimes(
    form: meds.Form,
    datetimes: List[datetime],
) -> None:
