...
def test_weekview_day_names(session, user, weekview_client, weekdays):
    response = weekview_client.get("/week/2021-1-3")
    soup = BeautifulSoup(response.content, "html.parser")
    day_divs = soup.find_all("div", {"class": "day-name"})
    for i in range(6):
        print(i)
        print(str(day_divs[i]))
        assert weekdays[i][:3].upper() in str(day_divs[i])


def test_weekview_day_dates(session, user, weekview_client, sunday):
    response = weekview_client.get("/week/2021-1-3")
    soup = BeautifulSoup(response.content, "html.parser")
...
def test_weekview_html_events(
    ...
    response = weekview_client.get(f"/week/{date}")
    soup = BeautifulSoup(response.content, "html.parser")
