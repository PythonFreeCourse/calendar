...
def test_disabling_no_event_user(session, no_event_user):
    ...
    future_events = list(
        ...
        .join(UserEvent)
        .filter(UserEvent.user_id == user1.id, Event.start > datetime.now()),
    )
...
def test_disabling_user_participating_event(session, user1, event1):
    """making sure only users who only participate in events
    can disable and enable themselves."""
    association = UserEvent(user_id=user1.id, event_id=event1.id)
    ...
    deleted_user_event_connection = (
        session.query(UserEvent)
        .filter(UserEvent.user_id == user1.id, UserEvent.event_id == event1.id)
        .first()
    )
