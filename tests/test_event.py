...
def test_not_check_change_dates_allowed(event):
    ...
    with pytest.raises(HTTPException):
        assert (evt.check_change_dates_allowed(
...
def test_update_event_with_category(today_event, category, session):
    ...
    updated_event = evt.update_event(event_id=today_event.id,
                                     event=data,
                                     db=session)
    assert 'successful' in updated_event.title
...
def test_can_show_event_private(event, session, user):
    ...
    null_attributes = [
        private_event.color, private_event.emotion, private_event.category_id
    ]
