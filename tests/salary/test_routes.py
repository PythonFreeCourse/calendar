...
def test_get_salary_categories_empty(
...
def test_get_salary_categories(
    salary_session: Session,
    wage: SalarySettings,
) -> None:
    # Code revision required after categories feature is added
...
def test_get_salary_categories_new(
    salary_session: Session,
    wage: SalarySettings,
) -> None:
    # Code revision required after categories feature is added
...
def test_home_page_redirects_to_new(salary_test_client: TestClient) -> None:
    response = salary_test_client.get(conftest.ROUTES["home"])
    assert response.ok
    assert conftest.MESSAGES["create_settings"] in response.text


@mock.patch("app.routers.salary.routes.get_current_user", new=get_current_user)
...
def test_create_settings(
    ...
    assert (
        utils.get_settings(salary_session, salary_user.id, category_id) is None
    )
    data = {
...
def test_empty_category_pick_redirects_to_new(
    ...
    response = salary_test_client.get(path)
    assert any(
        temp.status_code == status.HTTP_307_TEMPORARY_REDIRECT
...
def test_edit_settings(
    ...
    assert response.ok
    assert conftest.MESSAGES["view_salary"] in response.text
    assert settings != utils.get_settings(
        salary_session,
        wage.user_id,
        wage.category_id,
    )
