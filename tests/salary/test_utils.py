...
HOLIDAY_TIMES = [
    (
        ...
        datetime(2020, 1, 4, 1),  # Friday - Saturday
        (datetime(2020, 1, 4), datetime(2020, 1, 5)),
    ),
    (
        datetime(2020, 1, 4, 15),
        datetime(2020, 1, 5, 1),  # Saturday - Sunday
        (datetime(2020, 1, 4), datetime(2020, 1, 5)),
    ),
    (
        datetime(2020, 1, 5, 15),
        datetime(2020, 1, 6, 1),  # Sunday - Monday
...
SYNC_TIMES = [
    (
        datetime(2020, 1, 3, 15),
        datetime(2020, 1, 3, 22),
        datetime(2020, 1, 3, 18, 30),
        datetime(2020, 1, 4, 2),
        3.5,
    ),
    (
...
HOUR_BASIS = [
    (
        datetime(2021, 1, 4, 9),
        datetime(2021, 1, 4, 19),  # Regular shift
        config.REGULAR_HOUR_BASIS,
    ),
    (
        datetime(2021, 1, 4, 18),
        datetime(2021, 1, 5, 4),  # Night shift
...
WEEK_SHIFTS = [
    ...
    (
        (
            ...
            Event(
                start=datetime(2021, 1, 11, 9),
                end=datetime(2021, 1, 11, 17),
            ),
            Event(
                start=datetime(2021, 1, 12, 9),
                end=datetime(2021, 1, 12, 17),
            ),
            Event(
                start=datetime(2021, 1, 13, 9),
                end=datetime(2021, 1, 13, 18),
            ),
            Event(
                start=datetime(2021, 1, 14, 9),
...
MONTH_SHIFTS = [(False, 0.0), (True, 720.0)]
...
def create_month_shifts(
    start: datetime,
    end: datetime,
    add_sixth_day: bool = False,
) -> List[Event]:
    shifts = []
    for i in range(4):
        for j in range(6):
...
def test_get_night_times(
    wage: SalarySettings,
    date: datetime,
    prev_day: bool,
    night_times: Tuple[datetime, datetime],
) -> None:
...
@pytest.mark.parametrize("start, end, dates", HOLIDAY_TIMES)
def test_get_relevant_holiday_times(
    ...
    assert utils.get_relevant_holiday_times(start, end, wage) == dates
...
def test_get_hours_during_holiday(
    wage: SalarySettings,
    ...
    # Code revision required after Shabbat times feature is added
    assert (
        utils.get_hours_during_holiday(shift_start, shift_end, wage) == total
    )


@pytest.mark.parametrize("start, end, overtimes", HOLIDAY_OVERTIMES)
def test_adjust_overtime(
    wage: SalarySettings,
    start: datetime,
    end: datetime,
    overtimes: Tuple[float, float],
...
def test_get_event_by_category() -> None:
    # Code revision required after categories feature is added
    shifts = (
        Event(start=datetime(2021, 1, 10, 9), end=datetime(2021, 1, 10, 19)),
        Event(start=datetime(2021, 1, 11, 9), end=datetime(2021, 1, 11, 17)),
        Event(start=datetime(2021, 1, 12, 9), end=datetime(2021, 1, 12, 17)),
        Event(start=datetime(2021, 1, 13, 9), end=datetime(2021, 1, 13, 18)),
        Event(start=datetime(2021, 1, 14, 9), end=datetime(2021, 1, 14, 17)),
        Event(
            start=datetime(2021, 1, 15, 9),
            end=datetime(2021, 1, 15, 14, 58),
        ),
...
def test_get_month_end(
    year: int,
    month: int,
    month_times: Tuple[datetime, datetime],
) -> None:
...
def test_calc_transport(
    amount: int,
    daily_transport: float,
    total: float,
) -> None:
...
@mock.patch(
    "app.routers.salary.utils.get_event_by_category",
...
def test_calc_salary(
    ...
    wage: SalarySettings,
    overtime: bool,
    deduction: config.NUMERIC,
    salary: Dict[str, config.NUMERIC],
) -> None:
    # Code revision required after categories feature is added
    assert utils.calc_salary(2021, 1, wage, overtime, 0, deduction) == salary


def test_get_settings(salary_session: Session, wage: SalarySettings) -> None:
    assert utils.get_settings(salary_session, wage.user_id, wage.category_id)


@pytest.mark.parametrize("form, boolean", UPDATES)
def test_update_settings(
    salary_session: Session,
    wage: SalarySettings,
    form: Dict[str, str],
    boolean: bool,
) -> None:
