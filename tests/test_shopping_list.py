...
def test_if_product_deleted(home_test_client, product: ShoppingProduct, session):
    response = home_test_client.post("/shopping_product/delete", data={"shopping_product_id": product.id})
    assert response.status_code == 302
    with pytest.raises(NoResultFound):
        by_id(session, product.id)


def test_if_product_created(home_test_client, session, product: ShoppingProduct, test1):
    response = home_test_client.post("/shopping_product/add",
...
def test_if_product_edited(home_test_client, session, product: ShoppingProduct):
    response = home_test_client.post("/shopping_product/edit",
...
def test_if_product_has_bought(home_test_client, session, product: ShoppingProduct):
    response = home_test_client.post(f"/shopping_product/setBought/{product.id}",
                                     data={"shopping_product_id": product.id})
    assert response.status_code == 303
    response = home_test_client.get(f"/shopping_product/{product.id}", data={})
    content = response.content.decode('utf-8')
    content = json.loads(content)
    assert content['is_bought'] is True


def test_if_product_has_not_bought(home_test_client, session, product: ShoppingProduct):
    response = home_test_client.post(f"/shopping_product/setUnBought/{product.id}",
                                     data={"shopping_product_id": product.id,
                                           "session": session})
    assert response.status_code == 303
    response = home_test_client.get(f"/shopping_product/{shopping_product.id}", data={})
    content = response.content.decode('utf-8')
    content = json.loads(content)
